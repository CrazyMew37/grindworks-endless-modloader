...
func apply_cooked(chain: ModLoaderHookChain) -> void:
	if chain.reference_object.COOKED.id in chain.reference_object.manager.get_status_ids_for_target(chain.reference_object.targets[0]):
		...
		cooked.amount += (2 * ceili(Util.floor_number * 0.2))
	else:
		# Apply cooked fresh
		var cooked = chain.reference_object.COOKED.duplicate()
		cooked.amount = (10 * ceili(Util.floor_number * 0.2))
		cooked.rounds = -1
	...
	chain.reference_object.manager.affect_target(chain.reference_object.targets[0], (10 * ceili(Util.floor_number * 0.2)))
